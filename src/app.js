...
import { WALLPAPER_SOURCES, getBirdCategories, loadBirdWallpapers, searchWallpapers } from "./wallpapers.js";
...
function openSettings(){
  ...
  const wallpaperGrid=$(".wallpaper-grid"),categories=["全部",...new Set(WALLPAPERS.map(w=>w.category))];wallpaperGrid.insertAdjacentHTML("beforebegin",`<form class="wallpaper-search" id="wallpaperSearchForm"><input id="wallpaperKeyword" maxlength="40" placeholder="按 tag 搜索：落日、动漫、城市夜景……" aria-label="壁纸 tag 关键词"><button class="primary-button" type="submit">搜索壁纸</button></form><div class="wallpaper-search-hint">小鸟壁纸支持动态分类、热门 tag、关键词搜索及分页浏览</div><div class="wallpaper-sources">${WALLPAPER_SOURCES.map((source,i)=>`<button class="${i===0?"active":""}" data-wallpaper-source="${source.id}">${source.name}</button>`).join("")}</div><div class="bird-browser" id="birdBrowser"><label for="birdCategory">小鸟分类</label><select id="birdCategory"><option value="">正在加载分类…</option></select><div class="bird-tags" id="birdTags"><span>选择分类后显示热门 tag</span></div></div><div class="wallpaper-filters">${categories.map((c,i)=>`<button class="${i===0?"active":""}" data-wallpaper-filter="${c}">${c}</button>`).join("")}</div>`);wallpaperGrid.insertAdjacentHTML("afterend",`<div class="wallpaper-pager hidden" id="wallpaperPager"><button class="secondary-button" data-wallpaper-page="prev">上一页</button><span id="wallpaperPageInfo">第 1 / 1 页</span><button class="secondary-button" data-wallpaper-page="next">下一页</button></div>`);[...wallpaperGrid.children].forEach((card,index)=>{card.dataset.category=WALLPAPERS[index].category;card.insertAdjacentHTML("beforeend",`<em>${WALLPAPERS[index].category}</em>`)});
  let birdCategories=[],activeBirdRequest={mode:"latest",page:1};const wallpaperFilters=document.querySelector(".wallpaper-filters"),pager=$("#wallpaperPager");
  ...
  const loadBirdView=async request=>{activeBirdRequest={...activeBirdRequest,...request};wallpaperFilters.classList.add("hidden");wallpaperGrid.innerHTML='<div class="wallpaper-loading">正在加载小鸟壁纸…</div>';pager.classList.add("hidden");try{renderOnlineWallpapers(await loadBirdWallpapers(activeBirdRequest))}catch(error){wallpaperGrid.innerHTML=`<div class="wallpaper-error">${escapeHtml(error.message)}</div>`}};
  ...
  $("#birdCategory").addEventListener("change",e=>{const category=birdCategories.find(item=>item.id===e.target.value);$("#birdTags").innerHTML=category?.tags.length?category.tags.map(tag=>`<button data-bird-tag="${escapeHtml(tag)}"># ${escapeHtml(tag)}</button>`).join(""):'<span>该分类暂无热门 tag</span>';document.querySelectorAll("[data-wallpaper-source]").forEach(x=>x.classList.toggle("active",x.dataset.wallpaperSource==="bird"));loadBirdView(category?{mode:"category",categoryId:category.id,keyword:"",page:1}:{mode:"latest",categoryId:"",keyword:"",page:1});});
  $("#wallpaperSearchForm").addEventListener("submit",async e=>{e.preventDefault();const keyword=$("#wallpaperKeyword").value.trim();if(!keyword){toast("请输入壁纸 tag 或关键词");return}document.querySelectorAll("[data-wallpaper-source]").forEach(x=>x.classList.toggle("active",x.dataset.wallpaperSource==="bird"));activeBirdRequest={mode:"search",keyword,page:1,categoryId:""};wallpaperFilters.classList.add("hidden");wallpaperGrid.innerHTML='<div class="wallpaper-loading">正在搜索小鸟壁纸…</div>';pager.classList.add("hidden");try{renderOnlineWallpapers(await searchWallpapers(keyword,1))}catch(error){wallpaperGrid.innerHTML=`<div class="wallpaper-error">${escapeHtml(error.message)}</div>`}});
  $("#modalBody").addEventListener("click",async e=>{const tag=e.target.closest("[data-bird-tag]")?.dataset.birdTag;if(tag){$("#wallpaperKeyword").value=tag;activeBirdRequest={mode:"search",keyword:tag,page:1,categoryId:""};await loadBirdView(activeBirdRequest);return}const pageAction=e.target.closest("[data-wallpaper-page]")?.dataset.wallpaperPage;if(pageAction){const page=activeBirdRequest.page+(pageAction==="next"?1:-1);if(page>0)await loadBirdView({page});return}const sourceId=e.target.closest("[data-wallpaper-source]")?.dataset.wallpaperSource;if(sourceId){document.querySelectorAll("[data-wallpaper-source]").forEach(x=>x.classList.toggle("active",x.dataset.wallpaperSource===sourceId));const source=WALLPAPER_SOURCES.find(x=>x.id===sourceId);if(source.local){openSettings();return}wallpaperGrid.innerHTML='<div class="wallpaper-loading">正在加载壁纸…</div>';wallpaperFilters.classList.add("hidden");pager.classList.add("hidden");try{if(source.bird){activeBirdRequest={mode:"latest",page:1,categoryId:"",keyword:""};$("#birdCategory").value="";$("#birdTags").innerHTML='<span>选择分类后显示热门 tag</span>';}renderOnlineWallpapers(await source.load())}catch(error){wallpaperGrid.innerHTML=`<div class="wallpaper-error">${escapeHtml(error.message)}</div>`}return}const filter=e.target.closest("[data-wallpaper-filter]")?.dataset.wallpaperFilter;if(filter){document.querySelectorAll("[data-wallpaper-filter]").forEach(x=>x.classList.toggle("active",x.dataset.wallpaperFilter===filter));[...wallpaperGrid.children].forEach(card=>card.classList.toggle("filtered",filter!=="全部"&&card.dataset.category!==filter));return}const remoteUrl=e.target.closest("[data-wallpaper-url]")?.dataset.wallpaperUrl;if(remoteUrl){state.settings.wallpaper="";state.settings.customBackground=remoteUrl;await persist();applyAppearance();toast("壁纸已应用");return}const theme=e.target.closest("[data-theme]")?.dataset.theme;const wallpaper=e.target.closest("[data-wallpaper]")?.dataset.wallpaper;if(theme){state.settings.theme=theme;state.settings.wallpaper="";state.settings.customBackground="";await persist();applyAppearance();openSettings();}if(wallpaper){state.settings.wallpaper=wallpaper;state.settings.customBackground="";await persist();applyAppearance();openSettings();}});
