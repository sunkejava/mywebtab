import { CATEGORIES, DEFAULT_LINKS, DEFAULT_SETTINGS, SEARCH_ENGINES, THEMES } from "./data.js";
import { loadSettings, saveSettings } from "./storage.js";
import { fetchWeather } from "./weather.js";

const $ = (selector) => document.querySelector(selector);
const state = { settings: await loadSettings(), timer: null };
const quotes = [["保持好奇，持续创造。","MyWebTab"],["种一棵树最好的时间是十年前，其次是现在。","谚语"],["简单是可靠的先决条件。","Edsger Dijkstra"],["先完成，再完美。","行动准则"]];
...
function applyAppearance() {
  ...
  const background = state.settings.customBackground ? `url("${state.settings.customBackground}")` : theme.background;
  $("#backdrop").style.backgroundImage = background;
  document.documentElement.style.setProperty("--card-blur", `${state.settings.blur}px`);
  $(".veil").style.background = `rgba(5,9,18,${state.settings.shade / 100})`;
}
...
function renderLinks() {
  ...
  const links = state.settings.links.filter((link) => link.category === state.settings.category);
  $("#shortcutGrid").innerHTML = links.map((link) => `<a class="shortcut" href="${link.url}" title="${escapeHtml(link.name)}"><span class="shortcut-icon" style="--shortcut-color:${link.color}">${escapeHtml(link.icon || link.name.slice(0,2))}</span><span class="shortcut-name">${escapeHtml(link.name)}</span></a>`).join("") + `<button class="shortcut add-shortcut" id="addShortcut"><span class="shortcut-icon">+</span><span class="shortcut-name">添加网站</span></button>`;
}
...
function openSettings(){
  openModal("个性化设置","MyWebTab",`<div class="settings-group"><h3>主题背景</h3><div class="theme-grid">${THEMES.map(t=>`<button class="theme-swatch ${t.id===state.settings.theme?"active":""}" style="--theme-bg:${t.background}" data-theme="${t.id}" title="${t.name}"></button>`).join("")}</div></div><div class="settings-group"><h3>自定义背景</h3><div class="button-row"><button class="secondary-button" id="uploadBackground">选择本地图片</button><button class="danger-button" id="clearBackground">清除自定义背景</button></div><p class="eyebrow">图片仅保存在浏览器本地，建议小于 5MB</p></div><div class="settings-group"><h3>显示效果</h3><div class="field"><label>卡片模糊度：<span id="blurValue">${state.settings.blur}px</span></label><div class="range-line"><input id="blurRange" type="range" min="0" max="32" value="${state.settings.blur}"></div></div><div class="field"><label>背景遮罩：<span id="shadeValue">${state.settings.shade}%</span></label><div class="range-line"><input id="shadeRange" type="range" min="0" max="75" value="${state.settings.shade}"></div></div></div><div class="settings-group"><h3>数据管理</h3><div class="button-row"><button class="secondary-button" id="exportSettings">导出配置</button><button class="secondary-button" id="importSettings">导入配置</button><button class="danger-button" id="resetSettings">恢复全部默认</button></div></div>`);
  $("#modalBody").addEventListener("click",async e=>{const theme=e.target.dataset.theme;if(theme){state.settings.theme=theme;state.settings.customBackground="";await persist();applyAppearance();openSettings();}});
  $("#uploadBackground").onclick=()=>$("#backgroundPicker").click(); $("#clearBackground").onclick=async()=>{state.settings.customBackground="";await persist();applyAppearance();openSettings();};
...
const quote=quotes[new Date().getDate()%quotes.length];$("#quoteText").textContent=quote[0];$("#quoteAuthor").textContent=quote[1];
