...
function renderMarket(results = null) {
  ...
  if (!state.settings.watchlist.length) { list.innerHTML='<span class="market-empty">暂无盯盘项目，点击“配置”添加</span>'; return; }
  if (!results) { list.innerHTML=state.settings.watchlist.map(item=>`<div class="market-quote loading"><span>${escapeHtml(item.code)}</span><strong>${escapeHtml(item.name)}</strong><em>--</em></div>`).join(""); return; }
  list.innerHTML=results.map(result=>{if(result.status!=="fulfilled")return `<div class="market-quote failed"><span>${escapeHtml(result.item.code)}</span><strong>${escapeHtml(result.item.name)}</strong><em>暂不可用</em></div>`;const q=result.value,trend=q.changePercent>0?"up":q.changePercent<0?"down":"flat";return `<div class="market-quote ${trend}" title="${escapeHtml(q.name)}"><span>${escapeHtml(q.code)} · ${q.type==="fund"?"基金":"股票"}</span><strong>${escapeHtml(q.name)}</strong><em>${q.price.toFixed(q.price>=100?2:4)} <b>${q.changePercent>0?"+":""}${q.changePercent.toFixed(2)}%</b></em></div>`;}).join("");
}

async function refreshMarket() {
  renderMarket(); $("#marketUpdated").textContent="正在刷新…";
  const results=await fetchWatchlist(state.settings.watchlist);renderMarket(results);$("#marketUpdated").textContent=`更新于 ${new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:false})}`;
}

function openMarketEditor(existing=null){openModal(existing?"编辑盯盘":"添加盯盘","股票 · 基金",`<form id="marketForm"><div class="field-row"><div class="field"><label>类型</label><select name="type"><option value="fund" ${existing?.type!=="stock"?"selected":""}>基金</option><option value="stock" ${existing?.type==="stock"?"selected":""}>A 股</option></select></div><div class="field"><label>证券代码</label><input name="code" required maxlength="8" placeholder="例如 005693 / 600519" value="${escapeHtml(existing?.code||"")}"></div></div><div class="field"><label>显示名称</label><input name="name" required maxlength="40" placeholder="基金或股票名称" value="${escapeHtml(existing?.name||"")}"></div><button class="primary-button" type="submit">保存配置</button></form>`);$("#marketForm").onsubmit=async e=>{e.preventDefault();const data=new FormData(e.currentTarget),item=normalizeMarketItem({id:existing?.id||crypto.randomUUID(),type:data.get("type"),code:data.get("code"),name:data.get("name")});if(!item){toast("请输入正确的 6 位证券代码");return}state.settings.watchlist=existing?state.settings.watchlist.map(x=>x.id===existing.id?item:x):[...state.settings.watchlist,item];await persist();closeModal();renderMarket();refreshMarket();toast(existing?"盯盘配置已修改":"盯盘配置已添加");};}

function openManageMarket(){openModal("简易盯盘配置","股票 · 基金",`<div class="button-row"><button class="primary-button" id="addMarket">+ 添加股票/基金</button><button class="secondary-button" id="refreshMarketModal">刷新行情</button></div><div class="link-list market-manage-list">${state.settings.watchlist.map(item=>`<div class="link-row"><span class="market-type">${item.type==="fund"?"基":"股"}</span><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.code)}</small></div><div class="link-actions"><button data-market-edit="${item.id}">编辑</button><button class="remove-link" data-market-remove="${item.id}">删除</button></div></div>`).join("")||"<p>暂无盯盘项目。</p>"}</div>`);$("#addMarket").onclick=()=>openMarketEditor();$("#refreshMarketModal").onclick=()=>{closeModal();refreshMarket();};$("#modalBody").onclick=async e=>{const edit=e.target.dataset.marketEdit;if(edit){openMarketEditor(state.settings.watchlist.find(x=>x.id===edit));return}const remove=e.target.dataset.marketRemove;if(remove){state.settings.watchlist=state.settings.watchlist.filter(x=>x.id!==remove);await persist();renderMarket();openManageMarket();toast("盯盘项目已删除");}};}
...
function openSettings(){
  ...
  $(".data-actions .danger-zone").insertAdjacentHTML("beforebegin",`<article><div><strong>导入 Edge 收藏夹</strong><p>选择 Edge 导出的 HTML 文件，自动转换为快捷方式并按目录分类，重复网址将跳过。</p></div><button class="secondary-button" id="importBookmarksSettings">选择 HTML</button></article>`);
  ...
  $("#importSettings").onclick=()=>$("#importPicker").click(); $("#importBookmarksSettings").onclick=()=>$("#bookmarkPicker").click(); $("#resetSettings").onclick=async()=>{state.settings=structuredClone(DEFAULT_SETTINGS);await persist();location.reload();};
}
...
applyAppearance();renderClock();renderEngine();renderLinks();renderWeather();renderMarket();setInterval(renderClock,1000);
