...
function openSettings(){
  ...
  for(const key of ["blur","shade"]){$(`#${key}Range`).oninput=async e=>{state.settings[key]=Number(e.target.value);$(`#${key}Value`).textContent=`${e.target.value}${key==="blur"?"px":"%"}`;$("#effectPreview").style.setProperty(key==="blur"?"--preview-blur":"--preview-shade",key==="blur"?`${e.target.value}px`:String(Number(e.target.value)/100));applyAppearance();await persist();};}
